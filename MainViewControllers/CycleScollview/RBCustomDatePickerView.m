...
- (void)setTimeBroadcastView
{
    ...
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%@-%d-%d %@:%@",
                                   [dateString substringWithRange:NSMakeRange(0, 4)],
                                   monthInt
                                   ,dayInt
                                   ,[dateString substringWithRange:NSMakeRange(8, 2)]
                                   ,[dateString substringWithRange:NSMakeRange(10, 2)]];
...
- (void)scrollviewDidChangeNumber
{
    ...
    NSString *weekString = [self fromDateToWeek:dateString];
    nowPickerShowTimeLabel.text = [NSString stringWithFormat:@"%d-%d-%d %02d:%02d",yearInt,monthInt,dayInt,hourInt,minuteInt];
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:@"yyyy-MM-dd HH:mm"];
    NSString *selectTimeString = [NSString stringWithFormat:@"%d-%02d-%02d %02d:%02d",yearInt,monthInt,dayInt,hourInt,minuteInt];
    self.timeStr = selectTimeString;
