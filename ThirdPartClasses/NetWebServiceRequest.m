 //
//  NetWebServiceRequest.m
//  Ministry of culture
//
//  Created by ZhouLiang on 14-3-31.
...
@property (nonatomic, retain) __block ASIFormDataRequest* runningRequest;
...
{
    NSLog(@"回调的是:%@",[[self alloc] initWithUrl:WebURL SOAPActionURL:soapActionURL ServiceMethodName:strMethod SoapMessage:soapMsg]);
    return [[self alloc] initWithUrl:WebURL SOAPActionURL:soapActionURL ServiceMethodName:strMethod SoapMessage:soapMsg];
...

{
	...
	if ((self = [super init])) {
        self.runningRequest = [[ASIFormDataRequest alloc] initWithURL:url];
        ...
        [self.runningRequest setTimeOutSeconds:20.0];
...
- (void)startAsynchronous
{
    ...
    DEBUG_NSLOG(@"Url_path:%@",_runningRequest.url);
    DEBUG_NSLOG(@"Method:%@",_runningRequest.requestMethod);
    DEBUG_NSLOG(@"PostData:%@",_runningRequest.postBody);
    [_runningRequest startAsynchronous];
}


-(void) timeOutSecond:(NSTimeInterval)timer
{
    [self.runningRequest setTimeOutSeconds:timer];
}
