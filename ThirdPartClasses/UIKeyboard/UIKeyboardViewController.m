...
//找出所有的subview
- (NSArray *)allSubviews:(UIView *)theView {
    if ([theView respondsToSelector:@selector(subviews)])
    {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if ([self.boardDelegate respondsToSelector:@selector(alttextViewShouldChange:withRange:)]) {
		[self.boardDelegate alttextViewShouldChange:textField withRange:range];
	}
...
- (void)textViewDidChange:(UITextView *)textView {
    if ([self.boardDelegate respondsToSelector:@selector(alttextViewDidChange:)]) {
        [self.boardDelegate alttextViewDidChange:textView];
    }
}
// add - 2013-5-17 end
